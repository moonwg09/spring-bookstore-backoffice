...
<title>마당서점 백오피스 - 저자 관리</title>
<!-- 공통 다크모드 스타일 연동 -->
<link rel="stylesheet" href="${pageContext.request.contextPath}/resources/css/style.css">
<style>
    .sub-tabs { display: flex; gap: 10px; margin-bottom: 25px; border-bottom: 2px solid #333; padding-bottom: 15px; }
    .bio-text { text-align: left; max-width: 300px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
</style>
</head>
...
<main class="main-content">
    ...
    <div class="sub-tabs">
        ...
        <a href="/backoffice/admin/author/list" class="btn" style="background:#deff9a; color:#000; font-weight:bold; text-decoration:none;">✍️ 저자 관리</a>
    </div>

    <!-- 기존에 만드신 본문 영역 시작 -->
    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:15px;">
        <h3 class="section-title" style="margin:0;">✍️ 저자 목록 관리</h3>
        <a href="/backoffice/admin/author/register" class="btn" style="background:#deff9a; color:#000; font-weight:bold; text-decoration:none;">+ 새 저자 등록</a>
    </div>
    ...
    <table>
        ...
        <tbody>
            <c:forEach items="${list}" var="author">
                <tr>
                    <td>AUT-${author.author_id}</td>
                    <td><strong>${author.name}</strong></td>
                    <td class="bio-text">${not empty author.bio ? author.bio : '등록된 소개가 없습니다.'}</td>
                    <td class="admin-only">
                        <div style="display:flex; gap:5px; justify-content:center;">
                            <a href="/backoffice/admin/author/modify?author_id=${author.author_id}" class="btn" style="background:#ffc107; color:black; text-decoration:none;">수정</a>
                            
                            <form action="/backoffice/admin/author/delete" method="post" style="display:inline;">
                                <input type="hidden" name="author_id" value="${author.author_id}">
                                <button type="submit" onclick="return confirm('정말 이 저자를 삭제하시겠습니까?');" class="btn" style="background:#dc3545; color:white; border:none; cursor:pointer;">삭제</button>
                            </form>
                        </div>
                    </td>
                </tr>
            ...
            <c:if test="${empty list}">
                <tr>
                    <td colspan="4" style="padding:30px;">등록된 저자가 없습니다.</td>
                </tr>
            </c:if>
        </tbody>
    </table>
