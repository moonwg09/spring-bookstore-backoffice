...
<style>
    ...
    input[type="text"]:focus, select:focus { border-color: #deff9a; outline: none; }
...
<main class="main-content">
    ...
    <h3 class="section-title">🛠️ 카테고리 정보 수정</h3>
    
    <!-- 기존에 만드신 form 수정 로직 100% 그대로 유지 -->
    <form action="/backoffice/admin/category/modify" method="post" style="background:#1a1a1a; padding:30px; border-radius:12px; border:1px solid #333; max-width:600px;">
        
        <input type="hidden" name="category_id" value="${category.category_id}">
        
        <div class="form-group">
            <label>상위 카테고리 변경</label>
            <select name="parent_id">
                <option value="0" ${category.parent_id == null ? 'selected' : ''}>--- 없음 (대분류) ---</option>
                <c:forEach items="${categoryList}" var="cat">
                    <c:if test="${cat.parent_id == null && cat.category_id != category.category_id}">
        ...
        <div class="form-group" style="margin-top: 30px; margin-bottom: 0;">
            ...
            <a href="/backoffice/admin/category/list" class="btn-cancel">취소</a>
        </div>
