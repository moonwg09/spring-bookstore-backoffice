...
<main class="main-content">
    ...
    <!-- 4대 기준정보 이동을 위한 상단 서브 탭 (출판사 관리 활성화) -->
    <div class="sub-tabs">
    ...
    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:15px;">
        <h3 class="section-title" style="margin:0;">🏢 출판사 목록 관리</h3>
        <a href="/backoffice/admin/publisher/register" class="btn" style="background:#deff9a; color:#000; font-weight:bold; text-decoration:none;">+ 새 출판사 등록</a>
    </div>
    ...
    <table>
        ...
        <tbody>
            <c:forEach items="${list}" var="publisher">
                <tr>
                    <td>PUB-${publisher.publisher_id}</td>
                    <td><strong>${publisher.name}</strong></td>
                    <td style="color:#aaa;">${publisher.contact}</td>
                    <td class="admin-only">
                        <div style="display:flex; gap:5px; justify-content:center;">
                            <a href="/backoffice/admin/publisher/modify?publisher_id=${publisher.publisher_id}" class="btn" style="background:#ffc107; color:black; text-decoration:none;">수정</a>
                            
                            <form action="/backoffice/admin/publisher/delete" method="post" style="display:inline;">
                                <input type="hidden" name="publisher_id" value="${publisher.publisher_id}">
                                <button type="submit" onclick="return confirm('정말 이 출판사를 삭제하시겠습니까?');" class="btn" style="background:#dc3545; color:white; border:none; cursor:pointer;">삭제</button>
                            </form>
                        </div>
                    </td>
                </tr>
            ...
            <c:if test="${empty list}">
                <tr>
                    <td colspan="4" style="padding:30px;">등록된 출판사가 없습니다.</td>
                </tr>
            </c:if>
        </tbody>
    </table>
